...
final class HerdSharingConflictResolutionCommitBoundaryTests: XCTestCase {
  ...
  func testKeepOwnerCleanupCommitsBeforePostDiscardFailure() async throws {
    let container = try TestSupport.makeModelContainer()
    let herd = try insertHerd(in: container.mainContext)
    let journal = HerdSharingBridgeJournal(fileURL: makeJournalURL())
    let ownershipRegistry = ConflictResolutionBoundaryOwnershipRegistry()
    ...
    let guardService = HerdSharingCreationStateGuard(
      ...
      journal: journal,
      ownershipRegistry: ownershipRegistry,
      accountOwnershipRegistry: ConflictResolutionBoundaryAccountRegistry()
    )
    let resolver = ConflictResolutionBoundaryResolver(commitBeforeFailure: true)
    var participantReferenceRetired = false
    let repository = DeferredCoreDataHerdSharingRepository(
      repository: ConflictResolutionBoundaryRepository(),
      creationGuard: guardService,
      conflictResolver: resolver,
      discardedAcceptedParticipantProvenanceCleanup: { herdPublicID in
        XCTAssertEqual(herdPublicID, herd.publicID)
        participantReferenceRetired = true
      }
    )
    ...
    do {
      ...
      XCTAssertEqual(error as? ConflictResolutionBoundaryError, .postDiscardVerificationFailed)
    }

    XCTAssertTrue(resolver.didInvokeCommitCallback)
    XCTAssertTrue(participantReferenceRetired)
  }

  func testKeepOwnerCleanupDoesNotRunWhenDiscardFailsBeforeCommit() async throws {
    let container = try TestSupport.makeModelContainer()
    let herd = try insertHerd(in: container.mainContext)
    let ownershipRegistry = ConflictResolutionBoundaryOwnershipRegistry()
    ownershipRegistry.recordOwner(herdPublicID: herd.publicID, deviceID: "owner-device")
    let guardService = HerdSharingCreationStateGuard(
      context: container.mainContext,
      journal: HerdSharingBridgeJournal(fileURL: makeJournalURL()),
      ownershipRegistry: ownershipRegistry,
      accountOwnershipRegistry: ConflictResolutionBoundaryAccountRegistry()
    )
    let resolver = ConflictResolutionBoundaryResolver(commitBeforeFailure: false)
    ...
    let repository = DeferredCoreDataHerdSharingRepository(
      ...
      creationGuard: guardService,
      conflictResolver: resolver,
      discardedAcceptedParticipantProvenanceCleanup: { _ in
        participantReferenceRetired = true
      }
    )
    ...
    XCTAssertFalse(resolver.didInvokeCommitCallback)
    XCTAssertFalse(participantReferenceRetired)
  }
