...
final class PublicIDRepairReviewRound2RegressionTests: XCTestCase {
    func testExistingBridgeUsesRealImporterOnlyAfterLocalDuplicateIDsAreRepaired() async throws {
        ...
        defer { removeTemporaryJournalDirectory(for: journalURL) }
    ...
    private func verifyDuplicateHerdRecovery(
        ...
        XCTAssertFalse(
            exporter.exportedTargets.contains { $0.herdPublicID == duplicatedHerdID },
            "The canonical shared Herd bridge already has its manifest-final identity and must remain observation-only"
        )
        ...
        XCTAssertEqual(
            exporter.importedHerdIDs,
            [],
            "Creating the replacement Herd bridge does not require importing the unchanged canonical bridge"
        )
        XCTAssertEqual(repository.normalImportCallCount, 0)
        XCTAssertFalse(gate.requiresBridgeConvergence)
    }
    ...
    private func removeTemporaryJournalDirectory(for journalURL: URL) {
