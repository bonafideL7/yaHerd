...
final class FieldCheckAnimalQueryEngineTests: XCTestCase {
    ...
    func testSearchDoesNotFormatMissingDamTagAsUntagged() {
        let taggedWithoutDam = makeCheck(tag: "12")
        let untaggedAnimal = makeCheck(tag: "")

        let result = FieldCheckAnimalQueryEngine.apply(
            to: [taggedWithoutDam, untaggedAnimal],
        ...
        XCTAssertEqual(result.map(\.id), [untaggedAnimal.id])
    }
    ...
    func testAnimalTypeSortUsesGlobalDomainOrderAndTagTieBreaker() {
        let calf12 = makeCheck(tag: "12", animalType: .calf)
        let bull = makeCheck(tag: "5", animalType: .bull)
        let cow = makeCheck(tag: "4", animalType: .cow)
        let calf3 = makeCheck(tag: "3", animalType: .calf)
        let steer = makeCheck(tag: "2", animalType: .steer)
        let heifer = makeCheck(tag: "1", animalType: .heifer)

        let result = FieldCheckAnimalQueryEngine.apply(
            to: [bull, calf12, cow, steer, calf3, heifer],
            query: AnimalQuery(sortOrder: .animalType),
            formatTag: { number, _ in number }
        )

        XCTAssertEqual(
            result.map(\.animalType),
            [.calf, .calf, .heifer, .steer, .cow, .bull]
        )
        XCTAssertEqual(result.map(\.displayTagNumber), ["3", "12", "1", "2", "4", "5"])
    }
