...
final class HerdSharingAcceptedInvitationScopeTests: XCTestCase {
  ...
  func testAcceptedShareScopeRecoverableRetirementPersistsAcrossStoreRecreation() throws {
    let suiteName = "HerdSharingAcceptedInvitationScopeRetirement.\(UUID().uuidString)"
    let defaults = try XCTUnwrap(UserDefaults(suiteName: suiteName))
    defer { defaults.removePersistentDomain(forName: suiteName) }
    let storageKey = "pending-scopes"
    let scope = HerdSharingAcceptedShareImportScope(
      shareIdentifier: "retired-share",
    ...
    try store.record(scope)

    try store.removeRecoverably(scope)

    XCTAssertTrue(try store.pendingScopes().isEmpty)
    XCTAssertNil(store.immediateImportScope)
    let restartedStore = HerdSharingAcceptedShareImportScopeStore(
      defaults: defaults,
      storageKey: storageKey,
      currentAccountRecordNameProvider: { Self.participantAccountRecordName }
    )
    XCTAssertTrue(try restartedStore.pendingScopes().isEmpty)
    XCTAssertEqual(
      defaults.data(forKey: storageKey),
      try JSONEncoder().encode([HerdSharingAcceptedShareImportScope]())
    )
  }

  func testAcceptedShareScopeRetirementReadbackFailureRestoresBlockingScope() throws {
    let suiteName = "HerdSharingAcceptedInvitationScopeRetirementFailure.\(UUID().uuidString)"
    let defaults = try XCTUnwrap(UserDefaults(suiteName: suiteName))
    defer { defaults.removePersistentDomain(forName: suiteName) }
    let storageKey = "pending-scopes"
    let scope = HerdSharingAcceptedShareImportScope(
      shareIdentifier: "retirement-failure-share",
      rootRecordName: "retirement-failure-root",
      rootZoneName: "retirement-failure-zone",
      rootZoneOwnerName: "retirement-failure-owner",
      participantAccountRecordName: Self.participantAccountRecordName
    )
    ...
    let store = HerdSharingAcceptedShareImportScopeStore(
      ...
      storageKey: storageKey,
      currentAccountRecordNameProvider: { Self.participantAccountRecordName },
      persistenceVerifier: { defaults, key, expectedData in
        verificationCount += 1
    ...
    XCTAssertThrowsError(try store.removeRecoverably(scope)) { error in
      guard let actionError = error as? HerdSharingActionError,
            case .bridgeConsistencyFailed(let message) = actionError
      else {
        return XCTFail("Expected durable scope retirement failure, got \(error)")
      }
      XCTAssertTrue(message.contains("did not survive durable storage read-back"))
    }

    XCTAssertEqual(try store.pendingScopes(), [scope])
    XCTAssertEqual(store.immediateImportScope, scope)
    let restartedStore = HerdSharingAcceptedShareImportScopeStore(
      defaults: defaults,
      storageKey: storageKey,
      currentAccountRecordNameProvider: { Self.participantAccountRecordName }
    )
    XCTAssertEqual(try restartedStore.pendingScopes(), [scope])
  }

  func testCorruptRecoveryDurablyCommitsBackupAndMarkersBeforeRetiringActiveScope() throws {
    let suiteName = "HerdSharingAcceptedInvitationRecoveryDurability.\(UUID().uuidString)"
    let defaults = try XCTUnwrap(UserDefaults(suiteName: suiteName))
    defer { defaults.removePersistentDomain(forName: suiteName) }
    ...
    let backupKey = try store.backupAndResetCorruptStateForRecovery()

    XCTAssertNil(defaults.data(forKey: storageKey))
    XCTAssertEqual(defaults.data(forKey: backupKey), corruptData)
    XCTAssertEqual(
      defaults.string(forKey: "\(storageKey).corruptRecoveryBackupKey"),
      backupKey
    )
    XCTAssertTrue(defaults.bool(forKey: "\(storageKey).corruptRecoveryPending"))
    ...
    XCTAssertFalse(restartedStore.hasCorruptPersistedState)
    XCTAssertTrue(restartedStore.hasCorruptRecoveryPending)
    XCTAssertTrue(try restartedStore.pendingScopes().isEmpty)
  }

  func testCorruptRecoveryBackupReadbackFailureLeavesActiveScopeFailClosedAcrossRestart() throws {
    let suiteName = "HerdSharingAcceptedInvitationRecoveryReadbackFailure.\(UUID().uuidString)"
    let defaults = try XCTUnwrap(UserDefaults(suiteName: suiteName))
    defer { defaults.removePersistentDomain(forName: suiteName) }
    ...
    let store = HerdSharingAcceptedShareImportScopeStore(
      ...
      storageKey: storageKey,
      currentAccountRecordNameProvider: { "participant-account" },
      persistenceVerifier: { _, _, _ in false }
    )

    XCTAssertThrowsError(try store.backupAndResetCorruptStateForRecovery()) { error in
      guard let actionError = error as? HerdSharingActionError,
            case .bridgeConsistencyFailed(let message) = actionError
      else {
        return XCTFail("Expected a bridge consistency failure, received \(error).")
      }
      XCTAssertTrue(message.contains("could not be backed up durably"))
    }

    XCTAssertEqual(defaults.data(forKey: storageKey), corruptData)
    XCTAssertFalse(store.hasCorruptRecoveryPending)
    ...
    XCTAssertTrue(restartedStore.hasCorruptPersistedState)
    XCTAssertFalse(restartedStore.hasCorruptRecoveryPending)
    XCTAssertThrowsError(try restartedStore.pendingScopes()) { error in
      guard let actionError = error as? HerdSharingActionError,
            case .bridgeConsistencyFailed = actionError
      else {
        return XCTFail("Expected corrupt scope state to remain fail-closed, received \(error).")
      }
    }
  }

  func testInvitationScopeRecordFailsBeforeAcceptanceWhenDurableReadBackFails() throws {
    let suiteName = "HerdSharingAcceptedInvitationScopeReadBack.\(UUID().uuidString)"
    let defaults = try XCTUnwrap(UserDefaults(suiteName: suiteName))
    defer { defaults.removePersistentDomain(forName: suiteName) }
    let scope = HerdSharingAcceptedShareImportScope(
      shareIdentifier: "uncommitted-share",
      rootRecordName: "uncommitted-root",
      rootZoneName: "uncommitted-zone",
      rootZoneOwnerName: "uncommitted-owner",
      participantAccountRecordName: Self.participantAccountRecordName
    )
    let store = HerdSharingAcceptedShareImportScopeStore(
      defaults: defaults,
    ...
    XCTAssertThrowsError(try store.record(scope)) { error in
      guard let actionError = error as? HerdSharingActionError,
            case .bridgeConsistencyFailed(let message) = actionError
      else {
        return XCTFail("Expected durable scope persistence failure, got \(error)")
      }
      XCTAssertTrue(message.contains("did not survive durable storage read-back"))
    }
    XCTAssertNil(store.immediateImportScope)
  ...
  func testCachedPendingInvitationRootIsNotImportedWhenRemoteParticipationIsAbsent() async throws {
    let directory = makeDirectory(named: "cached-revoked-invitation")
    let suiteName = "HerdSharingCachedRevokedInvitation.\(UUID().uuidString)"
    let defaults = try XCTUnwrap(UserDefaults(suiteName: suiteName))
    defer {
      defaults.removePersistentDomain(forName: suiteName)
      try? FileManager.default.removeItem(at: directory)
    ...
    let sharedStore = try XCTUnwrap(store.sharedStore)
    let cachedHerd = try await seedHerd(
      name: "Revoked Cached Invitation",
      updatedAt: Date(timeIntervalSince1970: 200),
      into: store,
      sharedStore: sharedStore
    )
    let cachedRecord = try XCTUnwrap(
      try store.fetchSharedHerdRecords(publicID: cachedHerd.publicID, in: sharedStore).first
    )
    let rootRecordID = CKRecord.ID(
      recordName: "revoked-cached-root",
      zoneID: CKRecordZone.ID(zoneName: "revoked-cached-zone", ownerName: "revoked-owner")
    )
    recordIDsByObjectID[cachedRecord.objectID] = rootRecordID
    let scope = HerdSharingAcceptedShareImportScope(
      shareIdentifier: "revoked-cached-share",
    ...
    do {
      _ = try await store.importSharedRecordsIntoSwiftData(
        importer: SwiftDataHerdSharingActor(modelContainer: targetContainer)
      )
      XCTFail("Expected remotely revoked cached invitation data to remain blocked.")
    } catch let error as HerdSharingActionError {
      guard case .bridgeImportRequiresAccessVerification(let message) = error else {
        return XCTFail("Expected access verification failure, got \(error)")
      }
      XCTAssertTrue(message.contains("no accepted Herd root"))
    }
    ...
    XCTAssertTrue(try targetContainer.mainContext.fetch(FetchDescriptor<Herd>()).isEmpty)
    XCTAssertNotNil(try scopeStore.pendingScopes().first?.remoteAbsenceObservedAt)
  }
