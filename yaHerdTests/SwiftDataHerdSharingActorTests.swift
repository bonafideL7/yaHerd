...
final class SwiftDataHerdSharingActorTests: XCTestCase {
  ...
  func testExportBackfillsAndUsesDurableCollaborationMetadata() async throws {
    CollaborationRevisionRegistry.resetForTesting()
    defer { CollaborationRevisionRegistry.resetForTesting() }

    let container = try TestSupport.makeModelContainer()
    let context = container.mainContext
    let herd = Herd(name: "Legacy Metadata Herd")
    context.insert(herd)
    try context.save()
    ...
    let export = try await actor.makeExport(
      for: herd.toSummary(),
      storeDescription: "revision metadata backfill test"
    )

    let verificationContext = ModelContext(container)
    let records = try verificationContext.fetch(FetchDescriptor<CollaborationRevisionRecord>())
    let key = CollaborationAggregateKey(type: .herd, publicID: herd.publicID)
    let storedMetadata = try XCTUnwrap(records.first { $0.aggregateKey == key.storageKey })
    XCTAssertEqual(storedMetadata.revision, 1)
    XCTAssertEqual(storedMetadata.metadata.currentFieldValues["name"]?.encodedValue, herd.name)

    let exportedHerd = try XCTUnwrap(export.snapshot.records(for: .herd).first)
    XCTAssertEqual(exportedHerd.collaborationMetadata?.revision, 1)
    XCTAssertEqual(
      exportedHerd.collaborationMetadata?.currentFieldValues["name"]?.encodedValue,
      herd.name
    )
  }
}
