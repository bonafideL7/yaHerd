...
/// Every throwing application mutation that the current app exposes or the target Core Data architecture already
/// defines through a Domain transaction port.
///
...
/// mutation entry point from the post-commit boundary. Target-only transaction ports are included now so the
/// permanent contract protects the final write architecture rather than freezing only today's SwiftData-era surface.
/// If cutover intentionally removes a legacy public mutation entry point, remove its operation here in that same
/// change; do not add a compatibility adapter solely to keep an obsolete contract case executable.
enum MutationBoundaryContractOperation: String, CaseIterable, Sendable {
    ...
    case animalAggregateCreateTransaction
    case animalAggregateUpdateTransaction
    ...
    var expectedReason: DataMutationReason {
        ...
        case .animalCreate,
             ...
             .animalAggregateCreateTransaction,
             .animalAggregateUpdateTransaction:
            return .animal
...
/// a durable business-state change. For target-only operations, the Core Data runner must exercise
/// `AnimalAggregateTransactionWriting` or `PastureDeletionTransactionWriting` directly through the final mutation
/// publication boundary rather than routing them back through legacy repository writes. Failure probes must execute
/// that same entry point with valid Domain input and inject a persistence failure after the write reaches persistence
/// so the logical command throws instead of reporting success. A validation-only failure is not sufficient. Detailed
/// rollback behavior remains owned by the corresponding feature repository/transaction contract and is intentionally
/// not duplicated here. The control must expose
/// `didReachInjectedPersistenceFailure`: it remains false for a success probe, begins false for a failure probe, and
/// becomes true only when execution reaches the configured persistence fault. That lets this contract distinguish the
/// intended persistence failure from validation, missing-implementation, or other pre-persistence errors.
///
...
/// - Feature repository contracts own detailed durable success state, validation behavior, relationship/history
///   semantics, and feature-specific errors for current repository operations.
/// - `PersistenceTransactionRollbackContract` owns all-or-nothing rollback for the target Animal aggregate and Pasture
///   deletion transaction ports after a material partial write has been staged.
...
enum MutationBoundaryContract {
    static func assertSuccessfulLogicalWritesPublishExactlyOnceAfterDurableCommit(
        ...
        for operation in MutationBoundaryContractOperation.allCases {
            ...
            XCTAssertFalse(
                ...
                "Success fixture setup for \(operation.rawValue) must not report that the injected persistence fault was reached.",
                file: file,
                line: line
            )
            ...
            XCTAssertFalse(
                control.didReachInjectedPersistenceFailure,
                "A successful \(operation.rawValue) probe must not trip the configured persistence-failure injector.",
                file: file,
                line: line
            )
            XCTAssertEqual(
    ...
    static func assertFailedPersistenceWritesPublishNoSuccessEvent(
        ...
        for operation in MutationBoundaryContractOperation.allCases {
            ...
            XCTAssertFalse(
                control.didReachInjectedPersistenceFailure,
                "Fixture setup for \(operation.rawValue) must not report that the injected persistence fault was already reached.",
                file: file,
                line: line
            )
