...
enum FieldCheckRepositoryContract {
    static func assertSessionCreationAndReadProjections(
        ...
        let damCheck = try XCTUnwrap(
            detail.animalChecks.first { $0.animalID == dam.id },
            file: file,
            line: line
        )
        ...
        let calfCheck = try XCTUnwrap(
            detail.animalChecks.first { $0.animalID == calf.id },
            file: file,
            line: line
        )
    ...
    static func assertMutableCountsNotesAndRosterStateSurviveReload(
        ...
        let quickCountType = try XCTUnwrap(created.animalChecks.first?.animalType, file: file, line: line)

        try repository.updateQuickAnimalTypeCounts(sessionID: sessionID, counts: [quickCountType: 1])
        ...
        XCTAssertEqual(afterCountReload.quickAnimalTypeCounts[quickCountType], 1, file: file, line: line)

        let firstCheckID = created.animalChecks[0].id
        let secondCheckID = created.animalChecks[1].id
        try repository.setAnimalCheckCounted(sessionID: sessionID, animalCheckID: firstCheckID, isCounted: true)
        ...
        let counted = try XCTUnwrap(reloaded.animalChecks.first { $0.id == firstCheckID }, file: file, line: line)
        ...
        XCTAssertTrue(summary.animalChecks.contains { $0.id == firstCheckID && $0.wasCounted }, file: file, line: line)
    ...
    static func assertTrackedAnimalAdditionPersistsRosterAndDestination(
        ...
        let checkedAt = Date(timeIntervalSince1970: 30_500)
        ...
        let trackedCheck = try XCTUnwrap(
            reloaded.animalChecks.first { $0.animalID == tracked.id },
            file: file,
            line: line
        )
        ...
        try repository.addTrackedAnimalToSession(
            sessionID: sessionID,
            animalID: tracked.id,
            checkedAt: Date(timeIntervalSince1970: 30_600)
        )
    ...
    static func assertFindingLifecycleAndOpenFindingReader(
        ...
        let sessionID = try repository.createSession(
            input: FieldCheckSessionStartInput(
                pastureID: pasture.id,
                startedAt: Date(timeIntervalSince1970: 40_000),
                notes: "Finding lifecycle"
            )
        )
        let originalDate = Date(timeIntervalSince1970: 40_100)
        ...
        let updatedDate = Date(timeIntervalSince1970: 40_200)
        ...
        let pastureFindingDate = Date(timeIntervalSince1970: 40_300)
        try repository.addFinding(
            sessionID: sessionID,
            input: FieldCheckFindingInput(
    ...
    static func assertHistoricalSnapshotsSurviveLiveRecordChanges(
        ...
        let calfBefore = try XCTUnwrap(
            before.animalChecks.first { $0.animalID == calf.id },
            file: file,
            line: line
        )
        try repository.addFinding(
            sessionID: sessionID,
            input: FieldCheckFindingInput(
                recordedAt: Date(timeIntervalSince1970: 50_100),
                type: .pinkEye,
                severity: .warning,
                status: .open,
        ...
        let calfAfter = try XCTUnwrap(
            reloaded.animalChecks.first { $0.id == calfBefore.id },
            file: file,
            line: line
        )
    ...
    static func assertCompletionReopenAndEditLocking(
        ...
        try repository.addFinding(
            sessionID: sessionID,
            input: FieldCheckFindingInput(
                recordedAt: Date(timeIntervalSince1970: 60_100),
        ...
        assertThrowsRepositoryError(.sessionCompleted, file: file, line: line) {
            try completedRepository.addFinding(
                sessionID: sessionID,
                input: FieldCheckFindingInput(
                    recordedAt: Date(timeIntervalSince1970: 60_200),
                    type: .waterIssue,
                    severity: .warning,
                    status: .open,
        ...
        let reopened = try XCTUnwrap(
            reopenedRepository.fetchSessionDetail(id: sessionID),
            file: file,
            line: line
        )
    ...
    static func assertMissingRecordErrors(
        ...
        let sessionID = try repository.createSession(
            input: FieldCheckSessionStartInput(
                pastureID: pasture.id,
                startedAt: Date(timeIntervalSince1970: 70_100),
        ...
        assertThrowsRepositoryError(.animalNotFound, file: file, line: line) {
            try repository.addTrackedAnimalToSession(
                sessionID: sessionID,
                animalID: UUID(),
                checkedAt: Date(timeIntervalSince1970: 70_200)
            )
        }
    }
