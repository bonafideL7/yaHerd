...
final class PublicIDRepairBridgeDuplicateRecordRegressionTests: XCTestCase {
    ...
    func testRetainedDuplicateGroupTombstoneRequiresDiagnosticsResolutionWithoutDeletingAnimal() async throws {
        let fixture = try makeFixture()
        try await fixture.load()
        defer { fixture.removeStoreDirectory() }
        let local = try makeAnimalGraph(
        ...
        let localActor = SwiftDataHerdSharingActor(modelContainer: local.container)
        let contaminated = try await makeTombstonedBridge(
            fixture: fixture,
            local: local,
            targetID: fixture.retainedAnimalID
        )
        ...
        do {
            ...
            XCTFail("Expected retained-ID tombstone to require a diagnostics choice")
        } catch let error as PublicIDRepairBridgeResolutionRequired {
            let issue = try XCTUnwrap(error.issues.first)
            XCTAssertEqual(issue.kind, .canonicalRecord)
            XCTAssertEqual(issue.entityType, .animal)
            XCTAssertEqual(issue.fieldName, "deletionTargetPublicID")
            XCTAssertEqual(issue.referencedPublicID, fixture.retainedAnimalID)
            XCTAssertEqual(
    ...
    func testReplacementDuplicateGroupTombstoneDeletesKnownRepairedAnimal() async throws {
        let fixture = try makeFixture()
        try await fixture.load()
        defer { fixture.removeStoreDirectory() }
        let local = try makeAnimalGraph(
        ...
        let contaminated = try await makeTombstonedBridge(
            fixture: fixture,
            local: local,
            targetID: fixture.replacementAnimalID
        )
        ...
        XCTAssertEqual(animals.first?.publicID, fixture.retainedAnimalID)
        XCTAssertEqual(animals.first?.name, "Alpha")

        let unchangedBridge = try await fixture.store.readBridgeSnapshot(
            from: fixture.privateStore,
            requestedHerdPublicID: fixture.herdID,
            storeDescription: "replacement-tombstone-after-import"
        )
        XCTAssertEqual(unchangedBridge.records(for: .deletions).count, 1)
        XCTAssertEqual(
            unchangedBridge.records(for: .deletions).first?.parsedPublicID,
            fixture.replacementAnimalID
        )
    }

    private func makeTombstonedBridge(
        ...
        let targetKey = CollaborationAggregateKey(
            sourceEntityName: SharedAnimalRecord.entityName,
            publicID: targetID
        )
        let exportedAt = try XCTUnwrap(
            CollaborationRevisionRegistry.localMetadata(for: targetKey)?.modifiedAt
        )
        let deletedAt = exportedAt.addingTimeInterval(1)
        ...
        return try await fixture.store.readBridgeSnapshot(
            from: fixture.privateStore,
            requestedHerdPublicID: fixture.herdID,
            storeDescription: "repair-tombstone"
        )
    }
