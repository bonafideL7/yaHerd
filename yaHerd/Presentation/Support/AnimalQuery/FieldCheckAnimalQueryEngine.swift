    ...
    static func apply(
        ...
        if !searchText.isEmpty {
            result = result.filter { check in
                let damTagMatches = check.damDisplayTagNumber.map { damTagNumber in
                    damTagNumber.localizedCaseInsensitiveContains(searchText)
                        || formatTag(damTagNumber, check.damDisplayTagColorID)
                            .localizedCaseInsensitiveContains(searchText)
                } ?? false

                return check.displayTagNumber.localizedCaseInsensitiveContains(searchText)
        ...
        case .animalType:
            result.sort { left, right in
                let leftKey = AnimalQueryEngine.animalTypeSortKey(for: left.animalType)
                let rightKey = AnimalQueryEngine.animalTypeSortKey(for: right.animalType)
                if leftKey != rightKey {
