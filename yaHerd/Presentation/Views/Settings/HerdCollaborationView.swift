...
struct HerdCollaborationView: View {
    ...
    private var coreDataBoundarySection: some View {
        Section("Core Data Boundary") {
            ...
            LabeledContent("Bridge scope", value: "Herd root + pasture groups + pastures + animals + movement/status/health/pregnancy history")
            LabeledContent("SwiftData import", value: "Herd root + pasture groups + pastures + animals + movement/status/health/pregnancy history")

            Text("Core Data is intentionally isolated behind the sharing repository. SwiftData remains the app data store. The bridge now mirrors pasture groups, pastures, animals, movement records, status history, health records, and pregnancy checks into CloudKit sharing and can import those accepted shared records back into SwiftData. Working sessions, field checks, and support records still need bridge mirrors in later passes.")
                .font(.caption)
                .foregroundStyle(.secondary)
        }
    }
    ...
    private func sharedBridgeImportSection(
        ...
        Section("Shared Bridge Import") {
            ...
            Text("Use this after accepting a share or after the Core Data bridge receives remote shared changes. This pass imports the shared herd root, pasture groups, pastures, animals, movement records, status history, health records, and pregnancy checks into SwiftData so the normal app screens can display them.")
                .font(.caption)
                .foregroundStyle(.secondary)
        }
    }
    ...
    private func shareActionSection(sharingRepository: any HerdSharingRepository) -> some View {
        Section("Share Actions") {
            ...
            Text("This mirrors the current SwiftData herd, pasture groups, pastures, animals, movement records, status history, health records, and pregnancy checks into the isolated Core Data bridge, then opens Apple's CloudKit sharing sheet. It does not move yaHerd's normal app data out of SwiftData.")
                .font(.caption)
                .foregroundStyle(.secondary)
        }
    }
