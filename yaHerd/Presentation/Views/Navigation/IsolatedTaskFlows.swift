...
struct IdentityAwareFieldCheckSessionView: View {
    ...
    private func observeIdentityMutations() async {
        ...
        for await event in mutationStream.events(after: startingSequence) {
            ...
            case .sharedStoreImport:
                do {
                    let currentIdentity = try loadFieldCheckIdentity()
                    if FieldCheckSessionIdentitySnapshot.invalidatesPresentation(
                        refreshedSession: currentIdentity,
                        presentedFinding: presentedFindingIdentity,
                        presentedAnimalID: presentedAnimalID
                    ) {
                        resetIdentityBoundPresentation()
                        return
                    }
                } catch {
                    // A failed refresh does not prove the session or presented identity disappeared.
                    // Preserve draft state until a later mutation can be validated successfully.
                    continue
                }
            ...
            case .publicIDRepair:
                ...
                do {
                    _ = try loadFieldCheckIdentity()
                } catch {
                    // Do not discard transient editor state when the post-repair identity read itself
                    // failed. A later successful mutation can still revalidate the presentation.
                    continue
                }
                resetIdentityBoundPresentation()
                return
    ...
    @MainActor
    private func resetIdentityBoundPresentation() {
    ...
    @MainActor
    private func loadFieldCheckIdentity() throws -> FieldCheckSessionIdentitySnapshot? {
...
struct FieldCheckSessionIdentitySnapshot: Equatable {
    ...
    static func invalidatesPresentation(
        refreshedSession: FieldCheckSessionIdentitySnapshot?,
        presentedFinding: FieldCheckIdentityReference?,
        presentedAnimalID: UUID?
    ) -> Bool {
        guard let refreshedSession else { return true }
        return refreshedSession.invalidates(
            presentedFinding: presentedFinding,
            presentedAnimalID: presentedAnimalID
        )
    }
