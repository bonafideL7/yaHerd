...
actor DeterministicSwiftDataPublicIDRepairService: PublicIDRepairTransactionalService {
    #if DEBUG
    var backupDirectoryOverrideForTesting: URL?

    func setBackupDirectoryOverrideForTesting(_ url: URL?) {
        backupDirectoryOverrideForTesting = url
    }
    #endif
    ...
    func repair(
        ...
        guard plan.assessment.hasDuplicates || !plan.replacements.isEmpty else {
        ...
        let report = PublicIDRepairReport(
            ...
            bridgeCollisionResolutions: plan.bridgeCollisionResolutions.isEmpty
                ? nil
                : plan.bridgeCollisionResolutions
        )
