...
struct SyncRequestingAnimalRepository: AnimalRepository {
    ...
    func fetchAnimalDetail(id: UUID) throws -> AnimalDetailSnapshot? { try base.fetchAnimalDetail(id: id) }
    func fetchTimeline(id: UUID) throws -> [AnimalTimelineEvent] { try base.fetchTimeline(id: id) }
    func fetchStatusReferenceOptions() throws -> [AnimalStatusReferenceOption] { try base.fetchStatusReferenceOptions() }
...
struct SyncRequestingPastureRepository: PastureRepository {
    ...
    func fetchPastureDetail(id: UUID) throws -> PastureDetailSnapshot? { try base.fetchPastureDetail(id: id) }
    ...
    func nameExists(_ name: String, excluding id: UUID?) throws -> Bool { try base.nameExists(name, excluding: id) }
    ...
    func validatePastureGroupIDsExist(_ ids: [UUID]) throws { try base.validatePastureGroupIDsExist(ids) }
...
struct SyncRequestingFieldCheckRepository: FieldCheckRepository {
    ...
    func fetchOpenFindings(limit: Int) throws -> [FieldCheckFindingSnapshot] { try base.fetchOpenFindings(limit: limit) }
    func fetchSessionDetail(id: UUID) throws -> FieldCheckSessionDetailSnapshot? { try base.fetchSessionDetail(id: id) }
    ...
    func setAnimalCheckCounted(sessionID: UUID, animalCheckID: UUID, isCounted: Bool) throws {
        try base.setAnimalCheckCounted(sessionID: sessionID, animalCheckID: animalCheckID, isCounted: isCounted)
        scheduler.requestSharedDataSyncAfterMutation(reason: .fieldCheck)
    }

    func setAnimalCheckMissing(sessionID: UUID, animalCheckID: UUID, isMissing: Bool) throws {
        try base.setAnimalCheckMissing(sessionID: sessionID, animalCheckID: animalCheckID, isMissing: isMissing)
        scheduler.requestSharedDataSyncAfterMutation(reason: .fieldCheck)
    }

    func addTrackedAnimalToSession(sessionID: UUID, animalID: UUID, checkedAt: Date) throws {
        try base.addTrackedAnimalToSession(sessionID: sessionID, animalID: animalID, checkedAt: checkedAt)
        scheduler.requestSharedDataSyncAfterMutation(reason: .fieldCheck)
    }
...
final class SyncRequestingHerdRepository: HerdRepository {
    ...
    init(base: any HerdRepository, scheduler: HerdSharingMutationSyncScheduler) {
        self.base = base
...
final class SyncRequestingTagColorRepository: TagColorRepository {
    ...
    init(base: any TagColorRepository, scheduler: HerdSharingMutationSyncScheduler) {
        self.base = base
        self.scheduler = scheduler
    }
...
struct SyncRequestingWorkingRepository: WorkingRepository {
    ...
    func fetchSessionDetail(id: UUID) throws -> WorkingSessionDetailSnapshot? { try base.fetchSessionDetail(id: id) }
    func fetchTemplates() throws -> [WorkingProtocolTemplateSummary] { try base.fetchTemplates() }
    func fetchTemplateDetail(id: UUID) throws -> WorkingProtocolTemplateDetailSnapshot? { try base.fetchTemplateDetail(id: id) }
