...
`HerdRoute`, `WorkflowRoute`, `AppPresentedSheet`, `AppFullScreenWorkflow`, and `AppNavigationRequest` are typed `Codable` values. `RootAppView` persists an `AppNavigationSnapshot` in scene storage and restores it when the scene starts. URL routes are translated into the same typed request model used by in-app navigation entry points.
